     ...
     public void validate(FacesContext arg0, UIComponent arg1, Object valorTela) throws ValidatorException {
          if (valorTela != null 
        	  ...
        	  ResourceBundle bundle = ResourceBundle.getBundle( "messages" );
               ...
               message.setSummary(bundle.getString(MSG_ERRO_VALIDACAO_CPF));
     ...
     */
     private static boolean validaCPF(String cpf) {
    	 String cleanCpf = cpf.replace(".", "").replace("-", "");
          if (cpf == null || cpf.length() != 14 || isCPFPadrao(cleanCpf))
               return false;
 
          try {
               Long.parseLong(cleanCpf);
          } catch (NumberFormatException e) { // CPF não possui somente números
           return false;
          }
 
      return calcDigVerif(cleanCpf.substring(0, 9)).equals(cleanCpf.substring(9, 11));
     }

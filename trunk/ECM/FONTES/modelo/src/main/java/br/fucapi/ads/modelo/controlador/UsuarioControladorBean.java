...
//	private UsuarioTokenServico usuarioTokenServico;
	...
	public void init() {
		...
		if (this.usuarioTokenServico.listarTodos(token).getUtilizado()) {
			...
			try {
				request.setAttribute("token_utilizado", true);
	...
	public void incluir() throws RemoteException {
		...
		try {
			...
			context.addMessage(null, new FacesMessage(
					...
					"Operacacao realizado com sucesso", ""));
			...
			FacesContext.getCurrentInstance().addMessage(
					null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "Atenção. Problema ao realizar a operacao ",
							""));
		}
	}
	...
	public void isAdministrador(boolean isNovoUsuario) {
		...
		UsuarioGrupoLog usuarioGrupoLog = new UsuarioGrupoLog("ALFRESCO_ADMINISTRATORS", 
				this.usuario.getUserName(),
				((Usuario) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUserName(), 
				new Date(),	operacao);
	...
	public void incluirUsuarioGrupo() {
		...
		UsuarioGrupoLog usuarioGrupoLog = new UsuarioGrupoLog(this.grupoAlfresco.getDisplayName(), 
				this.usuario.getUserName(),
				((Usuario) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUserName(), 
				new Date(),	"I");
	...
	public void editarUsuario() throws RemoteException {
	...
	public String editar(Usuario usuario) {
		...
		try {
			...
			FacesContext.getCurrentInstance().addMessage(
					null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Problema ao realizar a operacao",
							""));
			return null;
	...
	/*public String modificarSenha() throws RemoteException {
		...
		//if (!listaToken.isEmpty() && listaToken.get(0).getUserName().equals(usuario.getUserName())	&& !listaToken.get(0).getUtilizado()) {
		if (listaToken != null && listaToken.getUserName().equals(usuario.getUserName())	&& !listaToken.getUtilizado()) {
			...
			if (dataAtual.after(futureDate)) {
				...
				UsuarioToken usuarioToken = listaToken;

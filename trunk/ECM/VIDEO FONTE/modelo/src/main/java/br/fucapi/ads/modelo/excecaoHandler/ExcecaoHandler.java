	...
	public void handle() throws FacesException {
		...
		while (i.hasNext()) {
			...
			try {
				...
				// (PaginaCentralControladorBean) fc
				...
				// ErroControladorBean erroControladorBean =
				// (ErroControladorBean) fc
				// .getApplication().evaluateExpressionGet(fc,
				// "#{erroControladorBean}",
				// ErroControladorBean.class);

				// MailService mailService = (MailService)
				// fc.getApplication().evaluateExpressionGet(fc,
				...
				// erroControladorBean.setEmail(((MailServiceImpl)
				// mailService).getSender());
